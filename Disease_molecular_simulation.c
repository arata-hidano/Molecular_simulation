...
int max_S3toS4_FMD = 150 ;
int max_S2toS3_FMD = 30;
...
double beta_a = 0.5;//within-herd transmission parameter
	   ...
	   //   printf("updating disease status") ;
		 ...
		 //     	printf("updated cancelled") ;
		  ...
		  //    printf("FarmData updated") ;
		    ...
		    //  printf("nth isolate is %d",*nth_isolate) ;
		      ...
		      //head_list_isolate[*current_id_isolate]->pro_id =(double)current_pro_id ;
		     // head_list_isolate[*current_id_isolate]->akey =(double)animal_akey ;
		  //  printf("akey is %lld",acurrent_isolatenimal_akey) ;
		  ...
		  //    printf("line 1364: pro id is %lld",current_pro_id );
		     // system("pause") ;
		   //  printf("err2") ;
		      animal_node_pointer[animal_akey]->ptr_isolate->isolated_time = (double)today_date;
		   //   printf("iso time is %f",today_date) ;
		...
		//	   printf("IsolateData stored") ;
			 ...
			 //  IsolateSeqData[*nth_isolate] = &(animal_node_pointer[animal_akey]->ptr_isolate->sequence);
			  // (*nth_isolate)++;
		//	   printf("S3 to S4 done, nth is %d", *nth_isolate) ;
...
//	 printf("A") ;
	...
	//	printf("B") ; 
		i = 0 ;
	 while(head_list_isolate[i]!=NULL)
   	    	 {
   	    	 free(head_list_isolate[i]);  
   	    	 i++;
   	    	 //free(IsolateSeqData[i]) ;
   		     	}
...
struct animal_node *inf_slaughtered[], struct isolate **head_list_isolate, struct inf_move *Farm_ptr_inf_move[])
{
	...
	long src_pro_id = current_event->src_pro_id ;
	int src_farm_id = (int)floor(src_pro_id/3) ;
...
int des_pro_id = current_event->des_pro_id;
	...
	if(current_status>=2)
	{
		FarmData[des_farm_id][current_status+5]++;
	}
	if(FarmData[des_farm_id][4]==0&&(current_status==1||current_status==2))
		{
...
/*----------------------------------------------------------------------------------*/
void move_production_type(double **FarmProductionStatus,struct animal_node *FarmProductionList[],int num_total_farms2,long long **FarmData )
{
struct animal_node *moving_animal;
struct animal_node *next_animal;
 ...
 for (i=0; i<num_total_farms;i++)
	...
	//	}
     ...
     //	target_animal = (struct animal_node*)malloc(sizeof( struct animal_node ));
		 ...
		 if(FarmData[i][7]!=(FarmProductionStatus[i*3][3]+FarmProductionStatus[i*3+1][3]+FarmProductionStatus[i*3+2][3]))
		      {
		      	printf("L2724: now FarmD is %lld and FPS is %f - %f - %f",FarmData[i][7],FarmProductionStatus[i*3][3],FarmProductionStatus[i*3+1][3],FarmProductionStatus[i*3+2][3]);
		      	system("pause") ;
			  }
  ...
  //   	printf("heifer ends");
	...
	//	}
		 ...
		 if(FarmData[i][7]!=(FarmProductionStatus[i*3][3]+FarmProductionStatus[i*3+1][3]+FarmProductionStatus[i*3+2][3]))
		      {
		      	printf("L2836: now FarmD is %lld and FPS is %f - %f - %f",FarmData[i][7],FarmProductionStatus[i*3][3],FarmProductionStatus[i*3+1][3],FarmProductionStatus[i*3+2][3]);
		      	system("pause") ;
			  }
	...
	//	 	printf("random inf is %ld", random_inf) ;
		 	...
		 	if(disease==1&&(current_animal->disease_status==2||current_animal->disease_status==1)) //if FMD infectious: for now if TB no passive detection occurs, if needed, add this: "(disease==0&&current_animal->disease_status>=2)||(" before hand and ")" in the end
			  ...
			  FarmData[farm_id][6] = 1 ; //becomes detected
			  	...
			  	if(floor(today_date)+30<sim_days)
			  	{
			  	struct event_node *adding_new_event;
			  			adding_new_event = (struct event_node*)malloc(sizeof( struct event_node ));
